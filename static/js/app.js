      ...
      <div class="product-card" style="animation-delay:${i*.07}s" onclick="openModal('${p.id}')">
        ...
        <div class="product-info">
          ...
          <div class="product-footer">
            ...
            <button class="add-btn" onclick="event.stopPropagation();addToCart('${p.id}','${p.name}','${p.emoji}')"><i class="fa-solid fa-cart-plus"></i></button>
          </div>
        </div>
      </div>`;
  }).join('');
...
async function loadRelatedProducts(productId) {
  ...
  try {
    ...
    container.innerHTML = items.map(item => `
      <div class="related-item" onclick="openModal('${item.id}')">
...
async function addFromModal() {
  ...
  for(let i=0;i<modalQty;i++) await fetch('/api/cart/add',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({product_id:selectedProduct.id})});
